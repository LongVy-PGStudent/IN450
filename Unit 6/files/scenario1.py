"""
Dressing Room Simulation - Scenario 1
10 customers, 3 dressing rooms, random items
"""
...
class DressingRooms:
    """Manages dressing rooms using a semaphore."""
    ...
    def requestRoom(self):
        """Customer requests for a dressing room. Blocked until one is available."""
        self.semaphore.acquire()

    def releaseRoom(self, usage_time):
        """Customer releases the dressing room."""
        with self._lock:
...
class Customer(threading.Thread):
    ...
    MAX_ITEMS = 6
    MIN_TIME_PER_ITEM = 1   # minutes (simulated as seconds for speed)
    MAX_TIME_PER_ITEM = 3
    ...
    def __init__(self, customer_id, dressing_rooms, clothing_items=0, results=None, results_lock=None):
        ...
        # ClothingItems=0 means random, 1-20 forces a specific count
        if clothing_items == 0:
    ...
    def run(self):
        ...
        for item in range(self.num_items):
            try_time = random.uniform(self.MIN_TIME_PER_ITEM, self.MAX_TIME_PER_ITEM) * 0.1  # scaled
            time.sleep(try_time)
...
class Scenario:
    ...
    def run(self):
        print(f"\n{'='*60}")
        print(f"SCENARIO 1: {self.num_customers} customers, {self.num_rooms} rooms, "
        ...
        for c in customers:
            c.start()
            time.sleep(random.uniform(0.01, 0.15))  # staggered arrivals
...
def scenario1():
    ...
    print("Using preset values for Scenario 1\n")

    num_rooms = 3
    num_customers = 10
    clothing_items = 0   # 0 = random
